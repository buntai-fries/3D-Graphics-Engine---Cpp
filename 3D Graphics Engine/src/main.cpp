...
bool LoadContent()
{
    ...
    // Load the shaders
    g_d3dVertexShader = LoadShader<ID3D11VertexShader>( L"../ShaderFiles/SimpleVertexShader.hlsl", "SimpleVertexShader", "latest" );
    g_d3dPixelShader = LoadShader<ID3D11PixelShader>( L"../ShaderFiles/SimplePixelShader.hlsl", "SimplePixelShader", "latest" );
...
ShaderClass* LoadShader(const std::wstring& fileName, const std::string& entryPoint, const std::string& _profile)
{
    ...
    if (FAILED(hr))
    {
        ...
        return NULL; // return NULL instead of false for shaderClass.
    }
